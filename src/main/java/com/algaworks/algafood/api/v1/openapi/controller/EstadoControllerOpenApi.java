...
public interface EstadoControllerOpenApi {
    ...
    @Operation(summary = "Lista as estados")
    CollectionModel<EstadoModel> listar();

    @Operation(summary = "Busca um estado por ID", responses = {
            @ApiResponse(responseCode = "200"),
            @ApiResponse(responseCode = "400", description = "ID do estado inválida", content = {
    ...
    EstadoModel adicionar(@RequestBody(description = "Representação de um novo estado", required = true)
    ...
    void remover(@Parameter(description = "ID de uma estado", example = "1", required = true) Long estadoId);
}
