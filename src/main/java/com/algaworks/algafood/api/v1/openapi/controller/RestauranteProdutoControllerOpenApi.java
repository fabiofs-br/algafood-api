...
public interface RestauranteProdutoControllerOpenApi {
    ...
    @Operation(summary = "Lista os produtos de um restaurante", responses = {
            ...
            @ApiResponse(responseCode = "400", description = "ID do restuarante inválida", content = {
    ...
    CollectionModel<ProdutoModel> listar(
            ...
            @Parameter(description = "Indica se devo ou não incluir produtos inativos no resultado da listagem")
            Boolean incluirInativos);

    @Operation(summary = "Busca um produto de um restaurante por ID", responses = {
            @ApiResponse(responseCode = "200"),
            @ApiResponse(responseCode = "400", description = "ID do restuarante ou produto inválida", content = {
                    @Content(schema = @Schema(ref = "Problema"))
            }),
            @ApiResponse(responseCode = "404", description = "Restaurante ou produto não encontrado", content = {
                    @Content(schema = @Schema(ref = "Problema"))
            })
    })
    ProdutoModel buscar(
